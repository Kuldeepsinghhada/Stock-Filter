      ...
      if (isTelegramEnabled) {
        for (var stock in newStocksForTelegram) {
          ...
          final double maxTradeAmount =
              await SharedPreferenceHelper.instance.getMaxTradeAmount();
          int quantity = 0;
          if (entryPrice > 0) {
...
''';
  ...
  static Future<List<HistoryModel>> buildTodayHistory(
    ...
    // 2️⃣ Group candles by date (DateTime key expected)
    final groupedByDate = CandleUtils.groupByDate(candles);

    final tradingDates = groupedByDate.keys.toList()..sort();

    if (tradingDates.isEmpty) return [];

    // ✅ IMPORTANT: derive "today" from data (NOT system time)
    final todayDate = tradingDates.last;
